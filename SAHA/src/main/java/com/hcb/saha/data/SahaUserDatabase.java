...
import com.hcb.saha.SahaConfig.Database;
...
public final class SahaUserDatabase {
    ...
    // TODO Replace with Guice injection of the helper (with auto context injection)
    ...
    private static User getUserFromCursor(Cursor cursor) {
    	...
    	user.setName(cursor.getString(cursor.getColumnIndex(UsersColumns.NAME_FIELD)));
    	user.setDirectory(cursor.getString(cursor.getColumnIndex(UsersColumns.DIRECTORY_FIELD)));
    	return user;
    }
    ...
    public static class SahaOpenHelper extends SQLiteOpenHelper {
        ...
        private void createUsersTable(SQLiteDatabase db) {
            final String query = new StringBuilder().append("CREATE TABLE ").append(USERS_TABLE).append(" (")
                    .append(UsersColumns.ID_FIELD).append(" INTEGER PRIMARY KEY AUTOINCREMENT, ")
                    .append(UsersColumns.NAME_FIELD).append(" TEXT, ").append(UsersColumns.DIRECTORY_FIELD)
